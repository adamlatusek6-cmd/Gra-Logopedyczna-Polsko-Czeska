function initPoziomSkrzynie(gameInstance) {
    ...
    function unlockWord(wordText) {
        ...
        if (!wordExists) {
            if (gameInstance.EngMode === false) gameInstance.showFeedback('❌ Tego słowa nie ma w grze. Spróbuj ponownie.', '#ef4444');
            else gameInstance.showFeedback('❌ This word is not in the game. Please try again.', '#ef4444');
            return false;
        }
        ...
        // Sprawdź czy nie jest już odblokowane
            ...
            if (gameInstance.EngMode === false) gameInstance.showFeedback(`⚠️ Słowo "${wordText}" jest już odblokowane!`, '#f59e0b');
            else gameInstance.showFeedback(`⚠️ The word "${wordText}" is already unlocked!`, '#f59e0b');
            return false;
        }
        ...
        // Odblokuj słowo
            ...
            if (gameInstance.EngMode === false) gameInstance.showFeedback(`🔓 Słowo "${wordText}" odblokowane! Teraz możesz je przesunąć.`, '#10b981');
            else gameInstance.showFeedback(`🔓 The word "${wordText}" is unlocked! You can now move it.`, '#10b981');
        }
    ...
    gameState.unlockedBoxes.add(boxId);
    console.log(`✅ Skrzynia "${boxId}" odblokowana!`);
    // Jeśli masz HTML element skrzyni, możesz dodać klasę CSS:
    const boxElement = document.querySelector(`[data-box-id="${boxId}"]`);
        if (boxElement) {
            boxElement.classList.remove('locked');
            boxElement.classList.add('unlocked');
        }
    }

    function isBoxLocked(boxId) {
        const box = boxCategories.find(b => b.id === boxId);
        return box && box.locked && !gameState.unlockedBoxes.has(boxId);
    }
    // ✅ OBSŁUGA ZMIANY JĘZYKA W REALTIME
        function updateLanguageDisplay() {
            const isEnglish = gameInstance.EngMode === true;
        ...
        function watchLanguageChanges() {
            ...
            if (angBtn) {
                // Obserwaj zmianę tekstu/klikniecia
            ...
            // Też wywołaj przy inicjalizacji
            updateLanguageDisplay();
        }
